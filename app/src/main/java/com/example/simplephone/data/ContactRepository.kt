...
class ContactRepository(private val context: Context) {
    ...
    fun getCallLogs(): List<com.example.simplephone.model.CallLogEntry> {
        val callLogs = mutableListOf<com.example.simplephone.model.CallLogEntry>()
        val contentResolver: ContentResolver = context.contentResolver
        ...
        cursor?.use {
            ...
            while (it.moveToNext()) {
                ...
                val callType = when (type) {
                    android.provider.CallLog.Calls.INCOMING_TYPE -> com.example.simplephone.model.CallType.INCOMING
                    android.provider.CallLog.Calls.OUTGOING_TYPE -> com.example.simplephone.model.CallType.OUTGOING
                    android.provider.CallLog.Calls.MISSED_TYPE -> com.example.simplephone.model.CallType.MISSED
                    else -> null
                }

                if (callType != null) {
                    callLogs.add(
                        com.example.simplephone.model.CallLogEntry(
                            id = id,
                            contactId = number, 
    ...
    fun getMissedCallsInLastHours(hours: Int): List<com.example.simplephone.model.CallLogEntry> {
        val allCalls = getCallLogs()
        val cutoffTime = java.time.LocalDateTime.now().minusHours(hours.toLong())
        return allCalls.filter { 
            it.type == com.example.simplephone.model.CallType.MISSED && it.timestamp.isAfter(cutoffTime)
        }
    }
}
